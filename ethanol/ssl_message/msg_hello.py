...
def send_msg_hello(server, id=0):
  """
  ...
  #print "send_msg_hello id:", id
  #1) create message
  msg_struct = Container(
                m_type = MSG_TYPE.MSG_HELLO_TYPE,
                m_id = id,
  ...
  #print "hello enviado para servidor > ", hex(msg)
  #2) sending message
  t0 = datetime.now()
  #print hex(msg)
  num_bytes=ssl_sock.write(msg)
  #print "num bytes enviados: ", num_bytes

  #3) retrieve server's response
  received_msg=ssl_sock.read(BUFFER_SIZE)
  if received_msg != '':
    t1 = datetime.now()
    #print "msg recebida > ", hex(received_msg)

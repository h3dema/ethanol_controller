...
from construct import SLInt32, ULInt64, CString
...
field_mac_sta = Struct('mac_sta',
                        ...
                        If(lambda ctx: ctx["mac_sta_size"] > 0,
                          CString("mac_sta")
                        ),
                  )
...
def get_association(server, id=0, association_type=None, mac_sta=None, mac_ap=None):
  ...
  """
  if (association_type == None) or (mac_sta==None) or (mac_ap == None):
    return None
  ...
  msg_struct = Container(
                ...
                p_version = VERSION,
                m_size = 0,
                mac_ap_size = 0 if mac_ap == None else len(mac_ap),
                mac_ap = mac_ap,
...
msg_event_association = Struct('msg_event_association',
       ...
       BooleanFlag('action'), # True: set the events, False: unset the events
     )

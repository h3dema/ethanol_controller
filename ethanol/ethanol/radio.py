...
from pox.ethanol.ssl_message.msg_channels import get_channels, get_currentchannel, set_currentchannel
...
from pox.ethanol.ssl_message.msg_powersave import set_powersave_mode, get_powersave_mode
...
class Radio(object):
  ...
  def __get_connection(self):
    """@return: tuple (ip, port_num) used for socket connection to the device"""
  ...
  def validChannels(self):
    """ informs a list of valid channel numbers, supported by the device in its wireless interface
      @return: the list of the channels that can be assigned to this interface
      return [] if an error occurs
    """
    server = self.__get_connection() # allows to send message to the AP
    msg, value = get_channels(server, id=self.msg_id, intf_name=self.__wiphy_name)
    return value
  ...
  def currentChannel(self):
    ...
    """
    if self.__wiphy_name == None:
      return -1
    server = self.__get_connection() # allows to send message to the AP
    msg, value = get_currentchannel(server, id=self.msg_id, intf_name=self.__wiphy_name)
    return value
  ...
  def currentChannel(self, new_channel):
    """
      ...
      @note: to confim that the channel was changed, issue currentChannel() command
    """
    server = self.__get_connection() # allows to send message to the AP
    set_currentchannel(server, id=self.msg_id, channel=new_channel, intf_name=self.__wiphy_name)
    log.debug("canal: %d interface: %s tcp: %d:%d", new_channel, self.__wiphy_name, server[0], server[1] )
  ...
  def channelInfo(self):
    ...
    @return: a list with channel info -- active_time, busy_time, channel_type, extension_channel_busy_time, frequency, in_use, noise, receive_time, transmit_time
    """
    # call ap to get information
    if self.__wiphy_name == None:
      return None
    server = self.__get_connection() # allows to send message to the AP
    msg, value = get_channelinfo(server, id=self.msg_id, intf_name=self.__wiphy_name, only_channel_in_use=True)
    return value
  ...
  def fastBSSTransition(self):
    ...
    msg, value = is_fastbsstransition_compatible(server, id=self.msg_id, intf_name=self.__wiphy_name)
    return value
  ...
  def _802_11b_Preamble(self):
    ...
    server = self.__get_connection()
    msg, value = get_preamble(server, id=self.msg_id, intf_name=self.__wiphy_name)
  ...
  def _802_11b_Preamble(self, value):
    ...
    """
    server = self.__get_connection()
    set_ap_preamble_req(server, id=self.msg_id, intf_name=self.__wiphy_name, preamble=value)
  ...
  def beaconInterval(self, value=100):
    ...
    """
    server = self.__get_connection()
    set_beacon_interval(server, value, id=self.msg_id, intf_name=self.__wiphy_name, beacon_interval=value)
  ...
  def getWirelessInterfaceInfo(self):
    ...
    """
    server = self.__get_connection() # allows to send message to the AP
    msg, value = req_wlan_info(server, id=self.msg_id, intf_name=self.__wiphy_name)
    return value

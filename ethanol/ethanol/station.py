...
def is_sta_with_ip_connected(ip):
    """
      @note: this is the ip of the STA's interface that sends packets to the
...
class Station(Device):
    ...
    def __del__(self):
        ...
        if self.__ip in list_of_stations and \
                self.intf_name in list_of_stations[self.__id]:
    ...
    def triggerTransition(self, new_vap):
        '''uses message MSG_TRIGGER_TRANSITION to send to the station a command
        to change to a new ap
    ...
    def __str__(self):
        ...
        return "Station %s %s conn(%s:%d)" % (self.intf_name,
                                              self.__mac_address, ip, port)

...
from pox.ethanol.ssl_message.msg_mean_sta_stats import send_msg_mean_sta_statistics
from pox.ethanol.ssl_message.msg_mean_sta_stats import send_msg_mean_sta_statistics_interface_remove, send_msg_mean_sta_statistics_interface_add
from pox.ethanol.ssl_message.msg_mean_sta_stats import send_msg_mean_sta_statistics_alpha, send_msg_mean_sta_statistics_time
...
"""
  ...
  __list_of_aps provides a list of all ethanol enabled aps connected to the controller
...
def get_ap_by_ip(ip):
    """
      ...
      @return: the AP object that has the provided ip address, or None if it doesn't exist
...
def get_vap_by_mac_address(mac_address):
  """
    ...
    @param mac_address: MAC address in dotted format of the Virtual AP (SSID)
  """
  vap = None
  if (len(__list_of_aps)) > 0:
    for ap_ip in __list_of_aps:
      ap = __list_of_aps[ap_ip]
      fvap = [vap for vap in ap.vaps if vap.mac_address == mac_address]
      if len(fvap) > 0:
        vap =  fvap[0]
...
def add_ap_openflow(ip):
  """
    ...
    inserts an entry in map_openflow_vs_ethanol_ip with the ip detected in pox.openflow.connection.
    when a Hello message arrives, AP.__init__() searchs this mapping and assings self to this entry

    @param ip: a string with the ip address in dotted format
    @type ip: str
"""
  if not ip in map_openflow_vs_ethanol_ip:
    map_openflow_vs_ethanol_ip[ip] = None # somente irá determinar na mensagem de hello


def add_ap( client_address ):
  """
    Create (and return) an AP object for the the device represented by the tuple client_address.
    This function updates a list of these objects.

    used by the Hello message's process

    @param client_address: tuple with (ip, port) used to make a socket connection to the AP
  ...
  if not(ip in __list_of_aps):
    ...
    log.info("Adding AP with IP %s to the list of connected aps (size %d)" % (ip, len(__list_of_aps)))
  else:
...
class AP(object):
  ...
  def __init__(self, ip, port = SERVER_PORT):
    ...
    """
    from pox.ethanol.ethanol.radio import Radio # import placed here to avoid 'import loop'
    from pox.ethanol.ethanol.network import Network

    #from ethanol.radio import Radio # import placed here to avoid 'import loop'
    ...
    """ retrieve and create radios (represented by the physical wifi interfaces)
    """
    msg, wlans = get_radio_wlans(server)
    intf_x_mac = {}

    log.info('wireless interfaces: [%s]' % "".join([_w['intf_name'] for _w in wlans]))
    if (None != wlans):
    ...
    if len(list_ssids) > 0:
      ...
      log.info("SSIDs in Radio: %s" % ",".join([_ssid['ssid'] for _ssid in list_ssids ]))
      ...
      for i in range(len(list_ssids)):
        ...
        if intf_name in self.__radios:
          # if there is no such ssid in list_of_networks (network.py) add it
          vap = self.createVirtualAP_and_insert_listVAP(ssid, # ssid
  ...
  def __get_connection(self):
    """
      ...
      @return: a tuple representing the socket to connection to the physical ap
    """
  ...
  def getSupportedInterfaceModes(self, interface_name):
    ...
    """
    server = self.__get_connection()
    return get_ap_supported_intf_modes(server, interface_name, id=self.msg_id)
  ...
  def enable_interface_stats(self):
    ...
    for intf in listWLAN_interfaces():
      send_msg_mean_sta_statistics_interface_add(server, id=self.msg_id, intf_name=intf)

  def disable_interface_stats(self):
    server = self.__get_connection()
    for intf in listWLAN_interfaces():
      send_msg_mean_sta_statistics_interface_remove(server, id=self.msg_id, intf_name=intf)

  @property
  def statistics_time(self):
    """time between collection of traffic statistics. -1 means that the collection is disabled"""
    return self.__stats_msec

  @statistics_time.setter
  def statistics_time(self, new_time):
    """
    @param new_time: set the time of collection in miliseconds. -1 means disabled
    """
    self.__stats_msec = new_time if new_time > 0 else -1
    server = self.__get_connection()
    send_msg_mean_sta_statistics_time(server, id=self.msg_id, msec=new_time)

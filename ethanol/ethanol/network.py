...
class Network(object):
  ...
  def __get_msg_id(self):
    """ returns the current ID number to be used in the message and increment this id by 1
    """
    id = self.__msg_id
    self.__msg_id += 1
    return id
  ...
  def SSID(self, newSSID, keepenabled = False):
    ...
    for vap in self.__listVAP:
      vap.ssid = newSSID
      if keepenabled: vap.enabled = True
  ...
  def handoffUser(station, new_vap):
    """ handles handoff. This method relies on 802.11 mobility domain feature.
        So the station and the AP should be configure to use mobility domain.
        This method disassociates the station from a vap in the network and moves it to a new_vap in this network.
        It also sends a message to the station, using station.triggerTransition(), instructing it to roam to a new ap.

        @see: documentacao-para-handover.pdf for instruction on how to set up the station and the AP for handover.
        **** not implemented yet ****

    """
    if not isInstance(station, Station):
      raise ValueError("station parameter must be a Station class!")
    if not isInstance(vap, VAP):
    ...
    if new_vap.connectNewUser(station):
      ...
      raise Exception("Cannot connect station "+station+" to ap "+new_vap)
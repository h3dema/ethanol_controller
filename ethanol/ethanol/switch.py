...
class LearningSwitch(object):
  ...
  def __init__(self, connection, transparent, idle_timeout = 10, hard_timeout = 30):
  ...
  def _handle_PacketIn(self, event):
    ...
    if not self.transparent: # 2
      if self.__packet.type == self.__packet.LLDP_TYPE or self.__packet.dst.isBridgeFiltered():
        __drop() # 2a
        return
      ...
      if dst not in self.macToPort: # 4
        ...
        if port == self.__in_port: # 5
          ...
          log.warning("Same port for packet from %s -> %s on %s.%s.  Drop."
              %(packet.src, dst, dpid_to_str(self.__dpid), port))
          __drop(10)
          return
        # 6

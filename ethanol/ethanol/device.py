...
class Device(object):
  ...
  @property
  def statistics(self):
    """ collect some cumulative statistics -- rx_packets, rx_bytes, rx_dropped, tx_packets, tx_bytes. this values are accumulate since the interface went up.
  ...
  def tx_bitrate(self, sta_mac=None):
    """ @return: the last seen tx_bitrate for a given station (in Mbps)
        or a list for each station connected (if sta_mac == None)
    """
    if (self.__intf_name == None) and \
      ((self.__mac_address == None) or (sta_mac == None) ):
      return -1
    else:
      server = self.get_connection
      if sta_mac != None:
